...
| Role | Visible Panels |
|---|---|
| [[QA Manager]] | Global panel + the 6 department panels |
| [[QA Operator]] | Only the panel for their assigned department |

## Global Panel — QA Manager

Consolidated view of the 6 supervised departments. Allows the [[QA Manager]] to quickly identify which departments require attention.

### Global Panel Charts

| # | Chart | Visualization Type | Data | Update |
|---|---------|----------------------|-------|---------------|
| 1 | **Current status of all 6 departments** | Summary cards (1 per department) | [[Quality Indicator]] per department + count of KPIs at Target / At Risk / Critical | Real time |
| 2 | **Quality Indicator trend** | Line chart (1 line per department) | History of [[Quality Indicator]] changes per department. X axis = weeks | Weekly |
...
|---------|----------------------|-------|
| **Zone coverage map** | Geographic map or zone diagram | The 6 [[Core/Catalogs/Zones|zones]] with [[Inspector]] availability indicator (active / temporary coverage / no coverage) |
...
| Chart | Visualization Type | Data |
|---------|----------------------|-------|
| **Associate Status Indicator distribution** | Donut chart | Percentage of associates in each of the 12 states of the [[Associate Status Indicator]] |

#### Sales

| Chart | Visualization Type | Data |
|---------|----------------------|-------|
| **Onboarding Status Indicator funnel** | Funnel chart | Number of hotels at each stage of the [[Core/Modules/Status Indicators/Onboarding Status Indicator|Onboarding Status Indicator]] (Gray → Light Blue → Green → Yellow → Pink → Orange) |

#### Recruitment

| Chart | Visualization Type | Data |
|---------|----------------------|-------|
| **Recruitment funnel** | Funnel chart | Interviewed candidates → approved → admitted to [[Associate Pool]] → assigned to position |
| **Pickup time heatmap by urgency** | Heat map | Requisition pickup time (Y axis) vs urgency level of the [[Core/Modules/Status Indicators/Requisition Urgency Indicator|Urgency Status Indicator]] (X axis). Intensity = number of requisitions |

#### Customer Service

> [!note]
> The specific charts for Customer Service will be defined once the department's KPIs are established. See [[QA/Metrics and KPIs by Department#Customer Service|Metrics and KPIs — Customer Service]].
